...
function progressMessage(elapsed: number) {
  if (elapsed >= 19_000) return "Todo listo";
  if (elapsed >= 15_000) return "Preparando aplicación";
  if (elapsed >= 10_000) return "Inicializando servicios";
  if (elapsed >= 5_000) return "Cargando configuración";
...
export function SplashScreen({ exiting = false }: { exiting?: boolean }) {
  ...
  const detail = exiting ? "Todo listo" : progressMessage(elapsed);
  return (
