...
type TdBridgePublisher struct {
	...
	DefaultCopies    int
	DiscType         string // Campo dinámico para CD, DVD, BD
	Format           string // Campo dinámico para UDF102, ISO9660, etc.
	Logger           *slog.Logger
}
...
func BuildJDF(job models.DiscJob, discType, format string, requestedCopies ...int) ([]byte, error) {
	...
	windowsViewerPath := filepath.Join(job.ViewerPath, "Symphony Viewer.exe")
	macOSViewerPath := filepath.Join(job.ViewerPath, "Symphony Viewer.app")
	paths := []string{job.DataPath, windowsViewerPath, macOSViewerPath, job.ManifestPath, autorunPath}
	...
	// Inyección dinámica de DISC_TYPE y FORMAT
	fmt.Fprintf(&out, "DISC_TYPE=%s\r\n", strings.ToUpper(discType))
	fmt.Fprintf(&out, "FORMAT=%s\r\n", strings.ToUpper(format))

	fmt.Fprintf(&out, "DATA=%s\tdata\r\n", job.DataPath)
	fmt.Fprintf(&out, "DATA=%s\tSymphony Viewer.exe\r\n", windowsViewerPath)
	fmt.Fprintf(&out, "DATA=%s\tSymphony Viewer.app\r\n", macOSViewerPath)
	fmt.Fprintf(&out, "DATA=%s\tstudy.dat\r\n", job.ManifestPath)
...
func (p *TdBridgePublisher) CreateJob(ctx context.Context, job models.DiscJob) (string, error) {
	...
	// Pasa DiscType y Format desde la estructura del struct TdBridgePublisher
	raw, err := BuildJDF(job, p.DiscType, p.Format, copies)
	if err != nil {
		return "", err
	}
	performance := config.EpsonConfig{PrintMode: p.PrintMode, LabelType: p.LabelType}
	if err := performance.ValidatePerformance(); err != nil {
		return "", err
	}
	if job.LabelPath != "" {
		if p.LabelType != 0 {
			raw = fmt.Appendf(raw, "LABEL_TYPE=%d\r\n", p.LabelType)
		}
		if p.PrintMode != 0 {
			raw = fmt.Appendf(raw, "PRINT_MODE=%d\r\n", p.PrintMode)
		}
	}
	// Omit WRITING_SPEED: TD Bridge selects the highest available speed.
